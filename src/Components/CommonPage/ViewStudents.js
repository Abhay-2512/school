...
function ViewStudents() {
    ...
    const search = (rows) => {
        let columns;
        const column1 = rows[0] && Object.keys(rows[0]);
        if(column1){
        columns=column1.filter((ele)=>ele!=="Dob"||ele!=="id");
        }else{
            columns=column1
        }
        // console.log(column1);
            ...
            return (
                columns.some(
                    (column) => {console.log(column);
                      return  ((column==="Age"||column==="Class")?(row[column].toString().includes(string.toString())):
                        ((column==="Dob")?(null):(row[column].toString().toLowerCase().includes(string.toLowerCase())))
                        )
                    }
                )
            )
    ...
    const genertePdf = () => {
        ...
        domToPdf(element, Options, () => {
            return alert("Are Sure To Download PDF");
        })
    }
    ...
    return (
        <div className="main">
            ...
            <div>
                <div className="fixheight" id="demo" >
                ...
                <div className="download">
                    <button type="button" onClick={genertePdf}  className="btndownload">Download Excel</button>
                </div>
            </div>
